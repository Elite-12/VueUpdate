  ...
  <div class="profile-head app-card mb-30">
    <div class="profile-top mt-4">
      <v-img
        ...
        :src="category.src ? category.src : 'https://placehold.co/150'"
        height="200px"
        contain
      >
        <v-card-title>
        ...
        <v-chip small label color="error" class="center mt-15" v-if="validationError !== true">
          {{ validationError }}
        </v-chip>
      </v-img>
    ...
    <v-dialog v-model="optionDialog" max-width="600px" @click:outside="closeOptionDialog">
      <v-card>
        <v-card-title> Category Option </v-card-title>
        <v-card-text>
          <v-container fluid grid-list-xl pt-0>
            <v-layout row wrap>
              <v-col xs12 md12 lg12 sm12 class="col-height-auto">
    ...
    >
      <v-card>
        <v-card-title> Are you sure you want to delete this option? </v-card-title>
        <v-card-actions>
          <v-btn color="error" @click.stop="deleteOption" :loading="loading" :disabled="loading">
            Delete
          </v-btn>
      ...
      rules: [
        (value) => {
          ...
          if (this.validationError !== true) {
            this.category.src = 'https://placehold.co/150'
            this.category.image = ''
          }
