<template>
  <div class="profile-top border-bottom-light-1 mt-4">
        ...
        <v-card-text>
          <v-container fluid grid-list-xl pt-0>
            <v-layout row wrap>
              <v-col xs12 md12 lg12 sm12 class="col-height-auto">
                <div>
                  <v-text-field v-model="selectedOptionValue.value" label="Name" filled>
                  </v-text-field>
                  <v-text-field v-model="selectedOptionValue.value_ar" label="Name Arabic" filled>
                  </v-text-field>
                </div>
              </v-col>
            </v-layout>
    ...
    >
      <v-table class="border-top-1 border-bottom-light-1">
        <thead>
          <tr>
        ...
        <draggable v-model="values" tag="tbody">
          <tr v-for="optionValue in values" :key="optionValue.id" class="border-top-1">
    ...
    >
      <v-card>
        <v-card-title> Are you sure you want to delete this option value? </v-card-title>
        <v-card-actions>
          <v-btn
            color="error"
...
export default {
  ...
  methods: {
    ...
    */
    createOptionValue() {
      this.loading = true
      this.$store.dispatch('createOptionValue', this.selectedOptionValue).finally(() => {
    ...
    updateOptionValue() {
      this.loading = true
      this.$store.dispatch('updateOptionValue', this.selectedOptionValue).finally(() => {
        this.closeOptionValueDialog()
      })
    },
    ...
    deleteOptionValue() {
      ...
      if (this.isNew) {
        ...
        this.$store.dispatch('deleteOptionValue', this.selectedOptionValue).finally(() => {
          this.closeOptionValueDialog()
        })
      }
    },
    ...
    deleteOptionLocally() {
      ...
      this.values = this.values.filter(function (item, index, arr) {
        return item.value !== itemToDelete.value && item.value_ar !== itemToDelete.value_ar
      })
    },
    ...
    */
    ...
    createOption() {
      if (this.isOptionNameValid() && this.isOptionValuesValid()) {
        this.loading = true
        this.$store.dispatch('createCategoryOption', this.option).finally(() => {
          this.loading = false
          this.close()
        })
      } else {
