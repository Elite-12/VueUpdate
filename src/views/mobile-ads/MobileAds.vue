  ...
  <div>
    ...
    <v-container fluid pt-0 grid-list-xl>
      ...
      <v-layout row wrap align-items-center search-wrap>
        <v-col xs12 sm10 md8 lg8 class="mx-auto">
          <div>
        ...
        <v-card>
          <v-card-title>
            ...
            <v-alert class="text-break" border="left" type="error"
              ><b class="text-uppercase">Warning:</b> Deleting ad will result in deleting it from
              all mobile screens if it appears in any. Note this action is
              <b class="text-uppercase">irreversible</b>
            </v-alert>
          </v-card-title>
          <v-card-actions>
            <v-btn color="error" @click.stop="deleteAd" :loading="loading" :disabled="loading">
              Delete
            </v-btn>
      ...
      <v-btn color="primary" @click="openNewDialog"> + Add New </v-btn>
    </v-container>
  </div>
</template>
<script>
