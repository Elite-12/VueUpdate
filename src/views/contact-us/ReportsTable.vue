  ...
  <div>
    ...
    <v-dialog v-model="dialog" max-width="500px" @click:outside="closeDialog">
      <v-card>
        ...
        <v-card-text>
          <v-text-field label="Title" required v-model="reply.title"></v-text-field>
          <v-textarea class="pt-2 pr-3" label="Body" v-model="reply.body"> </v-textarea>
          <v-btn
            @click="replyMessage"
            :disabled="loading"
            color="success"
    ...
    <v-dialog v-model="confirmDialog" max-width="500px" @click:outside="closeConfirmDialog">
      <v-card>
        ...
        <v-card-text>
          ...
          <v-btn class="float-right mt-2" color="warning" @click.stop="closeConfirmDialog"
            >Cancel</v-btn
          >
        </v-card-text>
      </v-card>
