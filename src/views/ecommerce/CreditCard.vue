<template>
  <div>
    ...
    <v-container fluid pt-0>
      <v-card>
        <v-card-actions class="pa-6 pb-0">
      ...
      <v-dialog v-model="addNewCardDialog" max-width="500">
        <v-card>
          <v-card-title>
            <span class="headline mb-4">{{ $t('message.addNewCard') }}</span>
          </v-card-title>
          <v-card-text>
            <span class="d-flex">
              <v-text-field
                ...
                required
                placeholder="#### - #### - #### - ####"
                v-model="newCardDetails.last4Digit"
                label="Card Number"
              ></v-text-field>
            </span>
            <span class="d-flex">
              <v-text-field
                name="cardHolderName"
                v-model="newCardDetails.cardHolderName"
                required
                label="Name"
              ></v-text-field>
            </span>
            <div class="d-flex">
              <v-col xs12 sm12 md4 mr-4>
                <v-autocomplete
                  name="months"
                  :items="getMonths()"
                  label="Expiry Month"
              ...
              <v-col xs12 sm12 md4 ;g4>
                <v-text-field
                  name="cvv"
                  v-model="newCardDetails.cvv"
                  :mask="maskCvv"
                  required
                  label="CVV"
                ></v-text-field>
          ...
          <v-card-actions class="px-6 pb-6">
            <v-spacer></v-spacer>
            <v-btn color="primary" @click="addNewCardDialog = false" class="px-4">Cancel</v-btn>
            <v-btn color="error" :disabled="ifEmptyFields()" @click="addNewCard" class="px-4"
              >Submit</v-btn
            >
          </v-card-actions>
        </v-card>
...
export default {
  ...
  methods: {
    ...
    ifEmptyFields() {
      const { last4Digit, cardHolderName, month, year, cvv } = this.newCardDetails
      if (
        last4Digit !== '' &&
