  ...
  <v-card>
            ...
            <div>
              ...
              <div class="profile-top mt-4">
                <v-img
                  ...
                  :src="selectedPackage.src ? selectedPackage.src : 'https://placehold.co/150'"
                  height="200px"
                  contain
                >
                  <v-card-title>
    ...
    <v-card-actions>
      <v-row justify="space-between" class="mb-4 pr-2 pl-2">
        <v-btn color="primary" @click.stop="save" :loading="loading" :disabled="loading"
          >Save
        </v-btn>
...
export default {
  ...
  methods: {
    ...
    clearImageInput() {
      this.changed = true
      this.selectedPackage.src = 'https://placehold.co/150'
    ...
    updatePackage() {
      this.appendFormDataParams()
      this.loading = true
      this.$store.dispatch('updatePackage', this.packageFormData).finally(() => {
        this.loading = false
        this.imageFile = null
        this.close()
      })
    ...
    appendFormDataParams() {
      ...
      this.packageFormData.append('duration_in_days', this.selectedPackage.duration_in_days)
      ...
      this.packageFormData.append('font_color', this.selectedPackage.font_color)
