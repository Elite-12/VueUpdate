<template>
  <div>
        ...
        <app-card customClasses="mb-30" colClasses="xl12 lg12 md12 sm12 xs12">
            ...
            <v-card>
              ...
              <v-card-text>
                <v-form v-model="form.valid" ref="form" lazy-validation>
                  ...
                  <v-btn class="float-right mt-2" color="warning" @click.stop="closeDialog"
                    >Cancel</v-btn
                  >
                </v-form>
              </v-card-text>
          ...
          <v-tooltip top v-if="!$can(Permissions.actions.create, Permissions.resources.admin)">
            <template v-slot:activator="{ on }">
              <span class="mt-70" @click="on">
                <v-btn disabled color="primary"> + {{ $t('message.adminAddNew') }} </v-btn>
              </span>
            </template>
            <span><v-icon icon="warning" /> You are not authorized to do this action</span>
          </v-tooltip>
    ...
    >
      <v-card>
        <v-card-title> Are you sure you want to delete this admin? </v-card-title>
        <v-card-actions>
          <v-btn color="error" @click.stop="deleteAdmin" :loading="loading" :disabled="loading">
            Delete
          </v-btn>
...
export default {
  ...
  methods: {
    ...
    submit() {
      if (this.$refs.form.validate()) {
        ...
        if (this.new_admin) {
          ...
          this.$store
            .dispatch('updateAdmin', { admin: admin, current_admin: false })
            .finally(() => {
              this.loading = false
              this.dialog = false
            })
    ...
    getAdmins() {
      ...
      this.$store.dispatch('getAdmins', this.options).finally(() => {
        this.loading = false
      })
    },
    ...
    deleteAdmin() {
      this.loading = true
      this.$store.dispatch('deleteAdmin', this.adminToDelete).finally(() => {
        this.loading = false
