    ...
    <app-card colClasses="xl12 lg12 md12 sm12 xs12">
      ...
      >
        <template v-slot:item.type="{ item }">
          <v-chip :color="item.type_color" dark>
            {{ item.type }}
          </v-chip>
        </template>
        ...
        <template v-slot:item.delete="{ item }">
          <v-btn
            class="mr-2"
            :disabled="loading || item.handled || !$can(UPDATE, RESOURCE)"
            :loading="loading"
            color="info"
            fab
            x-small
          ...
          <v-btn
            :disabled="loading || !$can(DELETE, RESOURCE)"
            :loading="loading"
            color="error "
            fab
            x-small
...
export default {
  ...
  watch: {
    options: {
      handler() {
        this.loading = true
        this.$store.dispatch('reports/fetchCategoryReports', this.options).finally(() => {
          this.loading = false
        })
      },
      deep: true,
  ...
  methods: {
    ...
    deleteReport() {
      ...
      this.$store
        .dispatch('deleteAdReport', this.selectedReport)
        .then(() => {
