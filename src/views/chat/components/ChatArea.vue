  ...
  <div class="chat-wrapper">
    ...
    <template v-else>
      <template v-if="selectedUser">
          ...
          <div class="chat-head-left d-custom-flex align-items-center">
            ...
            <div class="media align-items-center">
              <div class="media-left mr-4 pos-relative">
                ...
                />
                <span
                  :class="{ 'v-badge success rounded floating': selectedUser.status === 1 }"
                ></span>
              </div>
              <div class="media-body">
                <h6 class="mb-0">{{ selectedUser.name }}</h6>
                <span
                  class="grey--text fs-12 d-inline-block fw-normal"
                  v-if="selectedUser.status === 1"
                  >online</span
                >
                <span class="grey--text fs-12 d-inline-block fw-normal" v-else
                  >Last seen {{ selectedUser.lastSeen }}</span
                >
              </div>
            </div>
          </div>
        ...
        <PerfectScrollbar :style="getScrollHeight()" class="chat-area-scroll" :settings="settings">
          <div class="chat-body">
            <template v-for="(chat, index) in selectedUser.chats" :key="index">
              <div class="chat-block mb-4" :class="{ 'flex-row-reverse': chat.isAdmin }">
                <template v-if="!chat.isAdmin">
                  ...
                  <div class="chat-bubble-wrap">
                    ...
                    <span class="fs-12 grey--text text-left d-block mt-1 fw-normal">{{
                      chat.time
                    }}</span>
                  </div>
                </template>
