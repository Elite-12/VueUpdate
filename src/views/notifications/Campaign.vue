  ...
  <div class="profile-head app-card mb-30">
    <div class="profile-top mt-4">
      <v-img
        ...
        :src="campaign.src ? campaign.src : 'https://placehold.co/350x150.png'"
        height="200px"
        contain
      >
        <v-card-title>
      ...
      <div class="user-list-content">
        <div class="text-center">
          <v-col xs12 sm12 md12 lg12 pb-0>
            <div class="d-flex search-field-wrap justify-center">
              <div class="w-70">
                <v-text-field class="pt-0 pr-3" label="Title" v-model="campaign.title">
                </v-text-field>
                <v-text-field class="pt-2 pr-3" label="Title Arabic" v-model="campaign.title_ar">
                </v-text-field>

                <v-textarea class="pt-2 pr-3" label="Body" v-model="campaign.body"> </v-textarea>
                <v-textarea class="pt-2 pr-3" label="Body Arabic" v-model="campaign.body_ar">
                </v-textarea>
              </div>
            </div>
          </v-col>
...
export default {
  data() {
    return {
      ...
      imageFile: null,
      rules: [(value) => !value || value.size < 500000 || 'Avatar size should be less than 2 MB!'],
      rowStyle: {
        cursor: 'pointer',
      },
    }
  ...
  methods: {
    ...
    clearImageInput() {
      this.changed = true
      this.$refs.categoryImage.src = 'https://placehold.co/350x150.png'
      delete this.campaign.src
