<template>
  ...
  >
    <v-list three-line v-if="emails && emails.length > 0" class="list-aqua-ripple">
      <template v-for="mail in emails" :key="mail.id">
        <v-list-item @click="" ripple>
          ...
          <v-list-item-action>
            <v-list-item-action-text>
              <span class="grey--text fs-12 mb-2 d-block text-right fw-normal">{{
                mail.date
              }}</span>
    ...
    <div class="pa-4" v-else>
      <h3 class="text-center">No Emails Found</h3>
    </div>
...
export default {
  ...
  methods: {
    deleteEmail(selectedEmail) {
      let indexOfSelectedEmailInAllEmails = this.allEmails.indexOf(selectedEmail)
      let indexOfSelectedEmailInCurrentEmails = this.emails.indexOf(selectedEmail)
