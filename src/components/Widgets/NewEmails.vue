...
<template>
  <div>
    ...
    <PerfectScrollbar style="height: 540px" :settings="settings">
      <ul class="new-mail list-unstyled">
        <li v-for="email in emails" :key="email.id">
          <div class="d-flex flex-wrap justify-space-between align-items-center">
            <div class="d-flex justify-start">
              <v-avatar v-if="email.sender_avatar !== ''">
                <img class="img-responsive" :src="email.sender_avatar" alt="user" />
              </v-avatar>
              <template v-else>
          ...
          <template v-if="email.replyTextBox">
            <div class="d-custom-flex">
              <v-text-field v-model="messageReply" placeholder="Reply Message"></v-text-field>
    ...
    <v-dialog v-if="selectEmail !== null" v-model="viewEmailDialog" max-width="600">
      <v-list>
        <v-list-item class="py-2">
          <v-avatar v-if="selectEmail.sender_avatar !== ''">
            <img class="img-responsive" :src="selectEmail.sender_avatar" alt="user" />
          </v-avatar>
          <v-avatar v-else>
            <v-avatar class="teal">
